...
static void vblank(GB_gameboy_t *gb, GB_vblank_type_t type)
{
    ...
    if (osd_countdown && configuration.osd) {
        ...
        sameboy_draw_text(active_pixel_buffer,
...
restart:
    ...
    if (GB_is_inited(&gb)) {
        ...
        configuration.highpass_mode = GB_HIGHPASS_OFF;
        */
    ...
    */
	...
	char* battery_save_path = "save.sav";
	battery_save_path_ptr = battery_save_path;
    // GB_load_battery(&gb, battery_save_path);
    /*
...
bool get_arg_flag(const char *flag, int *argc, char **argv)
{
...
{
 	GB_set_key_state(&gb, GB_KEY_LEFT, left);
	GB_set_key_state(&gb, GB_KEY_RIGHT, right);
    GB_set_key_state(&gb, GB_KEY_UP, up);
    GB_set_key_state(&gb, GB_KEY_DOWN, down);
    GB_set_key_state(&gb, GB_KEY_A, a);
    GB_set_key_state(&gb, GB_KEY_B, b);
    GB_set_key_state(&gb, GB_KEY_SELECT, select);
    GB_set_key_state(&gb, GB_KEY_START, start);
}

int save_state(const char *path)
{
	//return GB_save_state(&gb, path);
	return GB_save_state_one_write(&gb, path);
}

int load_state(const char *path)
{
	return GB_load_state(&gb, path);
}
...
{
	GB_save_battery(&gb, path);
}

void gb_load_sram(const char *path)
{
	GB_load_battery(&gb, path);
}

void gb_set_audio_volume(uint8_t new_volume)
{
	configuration.volume = new_volume;
}