...
int GB_save_state_one_write(GB_gameboy_t* gb, const char* path)
{
    const size_t save_size = GB_get_save_state_size(gb);
    uint8_t* save_game = malloc(save_size);
    GB_save_state_to_buffer(gb, save_game);
    ...
    fwrite(save_game, 1, save_size, f);
    fclose(f);
    free(save_game);
    return 1;
}
