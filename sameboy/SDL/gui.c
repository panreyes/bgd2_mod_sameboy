...
void sameboy_draw_text(uint32_t *buffer, unsigned width, unsigned height, unsigned x, signed y, const char *string, uint32_t color, uint32_t border, bool is_osd)
{
...
static void draw_text_centered(uint32_t *buffer, unsigned width, unsigned height, unsigned y, const char *string, uint32_t color, uint32_t border, enum decoration decoration)
{
    ...
    sameboy_draw_text(buffer, width, height, x, y, string, color, border, false);
    switch (decoration) {
        case DECORATION_SELECTION:
            sameboy_draw_text(buffer, width, height, x - GLYPH_WIDTH, y, SELECTION_STRING, color, border, false);
            break;
        case DECORATION_ARROWS:
            sameboy_draw_text(buffer, width, height, x - GLYPH_WIDTH, y, LEFT_ARROW_STRING, color, border, false);
            sameboy_draw_text(buffer, width, height, width - x, y, RIGHT_ARROW_STRING, color, border, false);
            break;
        ...
        if (should_render) {
            ...
            switch (gui_state) {
                ...
                case SHOWING_MENU:
                    ...
                    break;
                case SHOWING_HELP:
                    sameboy_draw_text(pixels, width, height, 2 + x_offset, 2 + y_offset, help[current_help_page], gui_palette_native[3], gui_palette_native[0], false);
                    break;
